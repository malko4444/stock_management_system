...
// Safely fetch a collection for this admin; return [] on error instead of failing
const safeFetch = async (colName, adminId) => {
  try {
    const snap = await getDocs(
      query(collection(db, colName), where("adminId", "==", adminId))
    );
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
  } catch (err) {
    console.warn(`Fetch of ${colName} failed:`, err);
    return [];
  }
};
...
const Home = () => {
  ...
  const loadStats = useCallback(async () => {
    ...
    const [customers, inventory, employees, attendance] = await Promise.all([
      safeFetch("customers", adminId),
      safeFetch("inventory", adminId),
      safeFetch("employees", adminId),
      safeFetch("attendance", adminId),
    ]);
    ...
    const todaysAttendance = attendance
      .map((r) => ({
        ...r,
        timestamp: r.timestamp?.toDate ? r.timestamp.toDate() : null,
      }))
      .filter((r) => r.timestamp && r.timestamp >= start);

    const latestPerEmp = new Map();
    todaysAttendance.forEach((r) => {
      const prev = latestPerEmp.get(r.employeeId);
      if (!prev || prev.timestamp < r.timestamp) {
        latestPerEmp.set(r.employeeId, r);
      }
    });
    let presentToday = 0;
    latestPerEmp.forEach((v) => {
      if (v.type === "in") presentToday += 1;
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Overview of your customers, inventory and staff
            </p>
          </div>
          <Link
            to="/inventoryItem"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Active employees" value={loading ? "..." : stats.activeEmployees} icon={UserCog} tone="fuchsia" helper="On the kiosk list" />
          <StatCard label="Present today" value={loading ? "..." : stats.presentToday} icon={KeyRound} tone="emerald" helper="Currently checked in" />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <ActionCard to="/employees" title="Manage employees" description="Workers, hourly rates and PINs" icon={UserCog} />
            <ActionCard to="/kiosk" title="Attendance kiosk" description="Check in / out with 4-digit PIN" icon={KeyRound} />
            <ActionCard to="/employee-attendance" title="Attendance by employee" description="Per-worker day-by-day history" icon={CalendarDays} />
            <ActionCard to="/payroll" title="Monthly payroll" description="Hours \u00d7 rate, mark paid, PDF payslip" icon={Calculator} />
          </div>
        </div>
